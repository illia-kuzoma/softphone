...
class ReportAgentStatusesGroup extends ReportAgentStatuses
{
     ...
     * @var array
     */
    protected $fillable = ['agent_id', 'status_name', 'status_value', 'time_start', 'created_at', 'updated_at'];
    ...
    {
        $a_out = [];
        $a_last_agent_status = \DB::select("select agent_id, status_name, status_value, max(time_start) as last_time_start from agent_statuses_groups Group by agent_id, status_name ORDER BY agent_id;");
        if(count($a_last_agent_status))
        {
            foreach($a_last_agent_status as $status_data)
            {
                $a_out[$status_data->agent_id][$status_data->status_name][] = self::getBaseStructure($status_data->last_time_start, $status_data->status_value);
            }
        }
        return $a_out;
    }
